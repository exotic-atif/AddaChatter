...
function ensurePC(peerId){
  ...
  pc.onicecandidate = e=>{ if(e.candidate) send({type:'candidate', target:peerId, candidate:e.candidate}); };
  pc.ontrack = evt=>{
    let el = remoteEls[peerId];
    if(!el){
      const wrap=document.createElement('div');
      wrap.className='video-wrap';
      const v=document.createElement('video'); v.autoplay=true; v.playsInline=true; v.id='r-'+peerId; wrap.appendChild(v);
      const lbl=document.createElement('div'); lbl.className='label'; lbl.textContent=peerId.slice(0,6); wrap.appendChild(lbl);
...
function connectWS(room){
  ...
  ws.addEventListener('open', ()=>{ logStatus('connected'); send({type:'join', room}); });
  ws.addEventListener('message', async (evt)=>{
    ...
    if(data.type==='peer-left'){
      const id=data.id;
      if(pcs[id]){ try{ pcs[id].close() }catch{} delete pcs[id]; const el=document.getElementById('r-'+id); if(el && el.parentNode) el.parentNode.remove(); }
    }
  });
...
joinBtn.onclick = async ()=>{
  try{ if(!localStream) await startLocal(); connectWS(roomInput.value||'default'); logStatus('joining'); }catch(e){ alert('Camera/Mic access needed'); }
};

leaveBtn.onclick = ()=>{
  for(const k of Object.keys(pcs)){ try{ pcs[k].close() }catch{} delete pcs[k]; }
  remotes.innerHTML='';
  if(localStream){ for(const t of localStream.getTracks()) t.stop(); localStream=null; localVid.srcObject=null; }
...
camBtn.onclick = ()=>{
  ...
  camBtn.textContent=videoStopped?'Start Cam':'Stop Cam';
};
